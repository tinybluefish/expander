...
namespace Expander
{
    class Program
    {
        static string testA = @"Func0(Func1(Func2(Func3(Code1,Code2),1)),Func5(Code2,GetDataX('thing')))";
        ...
        static void Main(string[] args)
        {
            ...
            int pointer = 0;
            Node root = Node.ParseNode(testA, ref pointer, 0);
    ...
    class Node
    {
        private const string MAX_DEPTH = "maxdepth";
        private const string NODE_COUNT = "nodecount";
        ...
        public static Node ParseNode(string rootExpression, ref int pointer, int level)
        {
            Node node;
            ...
            if (leftParenFirst)
            {
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        Node child = ParseNode(rootExpression, ref pointer, level + 1);
            ...
            else if (rightParenFirst)
            {
                ...
                if (lookup.ContainsKey(node.name))
                {
                    ...
                    node.children.Add(ParseNode(lookup[node.name], ref subPointer, level + 1));
                }
            ...
            {
                node = new Expander.Node(-1, "WTFNode");
                Console.WriteLine("WTF?: how did this happen?");                
            }
